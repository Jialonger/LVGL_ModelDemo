...
void Lcd_Reset(void)
{
  GPIO_ResetBits(GPIOB,GPIO_Pin_14);//RST引脚输出为低
	delay_ms(100);
  GPIO_SetBits(GPIOB,GPIO_Pin_14);//RST引脚输出为高
	delay_ms(100);
}
...
/*
	x1 开始 x2 结束
*/
//void lcd_color_fill(uint16_t x1,uint16_t x2,uint16_t y1,uint16_t y2,uint16_t* Data)
//{
